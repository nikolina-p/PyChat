...
It is a web chat application leveraging Python's 'asyncio' and 'websocket' libraries.
...
Application architecture is designed according to MVC pattern. Also, Adapter pattern is used to add functionality to
...
SETUP INSTRUCTIONS:
    ...
    b. setup a database - run "python db_setup.py"
       - in terminal, navigate to the PyChat/Database folder and use command "python db_setup.py"
    ...
    c. load test data if you wish - run "load_test_data.py"
       ...
       - if you want an empty database, you will have to create users trough user interface (sign up option)
